...
namespace Sofka.Piguinera.Cotizacion.Services.Implementations
{
    public class TotalPriceQuotationService : ITotalPriceQuotationService
    {
        ...
        {
            UserPersistence userProvider = _databaseService.GetUser(payload.EmailProvider);

            if (userProvider != null)
            {
                BaseBookEntity bookEntity = _baseBookFactory.CreateBookEntity(payload.Type, payload.Id, payload.Title, payload.OriginalPrice, payload.EmailProvider, (int)userProvider.Seniority, payload.Quantity);
